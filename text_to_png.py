...
def font_start_checker(line):
    if not line.strip(): #line의 양쪽 공백을 지운다.
...
def txt_to_png(file_orig_path, file_full_path, width, height, sigma):
    ...
    with open(file_full_path, 'r') as lines: #lines는 리스트인것
        font_counter = 0
        not_data_checker = 0
        font_array = None
        font_name = ''
        ...
        for line in lines:
            if font_start_checker(line):  # endl
                ...
                ax = plt.Axes(fig, [0., 0., 1., 1.]) #하나의 그림

                ax.set_axis_off() #turn off the axis
                fig.add_axes(ax) #   ax = plt.Axes(fig, [0., 0., 1., 1.])을 그리는 것
                ax.imshow(font_blurred_array, aspect='auto', cmap='Greys') #blured array를 grey로 바꿔줌
